...
class MainController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        // 获取控制台一些信息 2018.7.4号
        $user_count  = Db::name('user')->where(['user_type'=>2,'user_status'=>1])->count();//注册会员
        $order_count = Db::name('order')->count();//订单总数
        $order_total = Db::name('order')->where(['pay_status'=>1])->whereOr(['pay_status'=>4])->sum('total_actual');//总金额
        $view_count  = Db::name('count')->count();//访问总次数


        $smtpSetting = cmf_get_option('smtp_setting');
        $this->assign('user_count',$user_count);  //会员总数
        $this->assign('order_count',$order_count);//订单总数
        $this->assign('order_total',$order_total);//订单总金额
        $this->assign('view_count',$view_count);  //访问总次数
