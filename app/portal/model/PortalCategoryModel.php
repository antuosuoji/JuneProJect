...
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['url']     = cmf_url('goods/List/index', ['id' => $item['id']]);;
    ...
    public function addCategory($data)
    {
        ...
        if ($result != false) {
            ...
            if (!empty($data['alias']) && !empty($id)) {
                $routeModel->setRoute($data['alias'], 'goods/List/index', ['id' => $id], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'goods/Article/index', ['cid' => $id], 2, 4999);
            }
    ...
    public function editCategory($data)
    {
        ...
        if (empty($oldCategory) || empty($newPath)) {
            ...
            $routeModel = new RouteModel();
            if (!empty($data['alias'])) {
                $routeModel->setRoute($data['alias'], 'goods/List/index', ['id' => $data['id']], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'goods/Article/index', ['cid' => $data['id']], 2, 4999);
            } else {
                $routeModel->deleteRoute('goods/List/index', ['id' => $data['id']]);
                $routeModel->deleteRoute('goods/Article/index', ['cid' => $data['id']]);
            }
    ...
    {
        $cats=  $this->field('more,name,id,parent_id')->where(['parent_id'=>0,'delete_time'=>0])->select();
        return $cats;
