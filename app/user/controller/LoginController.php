...
class LoginController extends HomeBaseController
{
    ...
    public function passwordReset()
    {
        ...
        if ($this->request->isPost()) {
            ...
            switch ($log) {
                case 0:
                    $this->success('密码重置成功',  "user/login/index");
                    break;
                case 1:
                    $this->error("您的账户尚未注册");
                    break;
    ...
    public function cleanCart()
    {
         ...
         $cart_cookie=unserialize(Cookie::get('cart'));
         if($cart_cookie && count($cart_cookie) > 0)
         {
            foreach ($cart_cookie as $key => $value)
            {
             $c_arr=explode('---',$value);
             $carts[]=$c_arr;
            }
            foreach($carts as $v)
            {
              ...
              $cart_data['goods_id']=$v[0];
              $cart_data['goods_attr_id']=$v[1];
              $cart_data['goods_attr_value']=$v[2];
              $cart_data['num']=$v[3];
              Db::name('cart')->insert($cart_data);
            }
            Cookie::delete('cart');
         }
